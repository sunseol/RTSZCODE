...
export class ZaloOAuthError extends Error {
  constructor(stage, message, code = null) {
    super(message);
...
export function parseZaloAuthError(rawError) {
...
export async function fetchZaloProfile(
  ...
  const relayUrl = transport.relayUrl ?? process.env.ZALO_RELAY_URL?.trim();
  const relaySecret = transport.relaySecret ?? process.env.ZALO_RELAY_SECRET?.trim();
  const proxyUrl = transport.proxyUrl ?? process.env.ZALO_PROXY_URL?.trim();

  if (relayUrl && !relaySecret) {
    throw new ZaloOAuthError('configuration', 'Zalo relay secret is missing');
  }
  ...
  const dispatcher = !relayUrl && proxyUrl ? new ProxyAgent(proxyUrl) : null;
  const request = dispatcher && fetchImpl === fetch ? undiciFetch : fetchImpl;
  const requestUrl = relayUrl || profileUrl.toString();
  const requestInit = relayUrl
    ? {
        method: 'POST',
        headers: {
  ...
  try {
    const response = await request(requestUrl, requestInit);
    const payload = await response.json();
    ...
    if (
      ...
      const errorCode = isRecord(payload) && Number.isInteger(payload.error)
        ? payload.error
        : null;
      throw new ZaloOAuthError('profile', 'Zalo profile request failed', errorCode);
    }
